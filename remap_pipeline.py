...
from cogvideox-controlnet.latent_lrm import LongLRM
from cogvideo_controlnet import CogVideoXControlnet
import torch.nn as nn


class LatentAlign3D(nn.Module):
    def __init__(self, channels):
        """
        Args:
            channels: C (dlatent/vlatent的通道数)
            spatial_size: W,H (空间维度大小)
        """
        super().__init__()
        ...
        self.conv_layers = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv3d(channels*2, channels, (3, 3, 3),
                      padding=(1, 1, 1))  # 最后恢复原始通道数
        )
    ...
    def forward(self, x):
        # 输入输出都是 (B,C,V,W,H)
        return self.conv_layers(x) + self.skip(x)
...
def resize_for_crop(image, crop_h, crop_w):
    ...
    resized_image = transforms.functional.resize(
        image, (out_h, out_w), antialias=True)
...
def prepare_frames(input_images, video_size, do_resize=True, do_crop=True):
    ...
    images_tensor = torch.from_numpy(
        input_images).permute(0, 3, 1, 2) / 127.5 - 1
    if do_resize:
        images_tensor = [resize_for_crop(
            x, crop_h=video_size[0], crop_w=video_size[1]) for x in images_tensor]
    if do_crop:
        images_tensor = [transforms.functional.center_crop(
            x, video_size) for x in images_tensor]
...
class ControlnetCogVideoXPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def __init__(
        ...
        lrm: LongLRM,
        latent_mapper: LatentAlign3D,
    ):
        super().__init__()

        self.register_modules(
            tokenizer=tokenizer, text_encoder=text_encoder, vae=vae, transformer=transformer, controlnet=controlnet, scheduler=scheduler, lrm=lrm, latent_mapper=latent_mapper
        )
    ...
    def prepare_controlnet_frames(self, controlnet_frames, height, width, do_classifier_free_guidance):
        ...
        controlnet_encoded_frames = prepared_frames.to(
            dtype=self.vae.dtype, device='cuda')
        controlnet_encoded_frames = torch.cat(
            [controlnet_encoded_frames] * 2) if do_classifier_free_guidance else controlnet_encoded_frames
    ...
    def check_inputs(
        ...
        if controlnet_frames is not None and controlnet_latents is not None:
            raise ValueError(
                "Only one of `controlnet_frames` or `controlnet_latents` should be provided")
    ...
    def __call__(
        ...
        max_sequence_length: int = 226,
        controlnet_weights: Optional[Union[float,
                                           list, np.ndarray, torch.FloatTensor]] = 1.0,
        ...
        if controlnet_latents is None:
            ...
            if duplicate_frames_count > 0:
                ...
                reversed_chunks_count = num_frames // len(
                    controlnet_sum_frames)
            ...
            controlnet_latents = torch.cat(
                [controlnet_latents] * 2) if do_classifier_free_guidance else controlnet_latents
        ...
        nvideo = self.video_processor.postprocess_video(
            video=video, output_type='pil')
        # Offload all models
        self.maybe_free_model_hooks()
        rescale_latents = self.latent_mapper(latents.permute(0, 2, 1, 3, 4))
        rvideo = self.decode_latents(rescale_latents.permute(0, 2, 1, 3, 4))
        renderings = self.lrm((1, num_frames, 3, height, width),
                              image_latent=rescale_latents, input_c2ws=c2ws, input_intr=intrinsics)
        renderings = renderings.clamp(0, 1)  # (B,  3, V, H, W)
        return video, nvideo, rvideo, renderings
