...
export MODEL_PATH="/jfs/qian.ren/huggingDown/${model_name}"
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
video_root_dir="/jfs/qian.ren/latentLRM/data/dataset" # subfolders: annotations/ pose_files/ video_clips/
annotation_json="annotations/train.json"

dir=`pwd`
output_dir="${dir}/out/${model_name}$(date +%Y%m%d)"

cd training

# If you are not using 8 gus, change `accelerate_config_machine_single.yaml` num_processes as your gpu number
accelerate launch --config_file accelerate_config_machine_8gpu.yaml  \
  train_controlnet.py \
  ...
  --stride_max 3 \
  --hflip_p 0.0 \
  --controlnet_transformer_num_layers 21 \
