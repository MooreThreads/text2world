...
def get_args():
    parser = argparse.ArgumentParser(
        description="Simple example of a training script for CogVideoX.")
...
def log_validation(
    ...
    for _ in range(args.num_validation_videos):
        video = pipe(**pipeline_args, generator=generator,
    ...
    for i, video in enumerate(videos):
        ...
        filename = os.path.join(
            args.output_dir, f"{epoch}_video_{i}_{prompt}.mp4")
...
def main(args):
    ...
    if args.init_from_transformer:
        ...
        print(
            f'[ Weights from transformer was loaded into controlnet ] [M: {len(m)} | U: {len(u)}]')

    if args.pretrained_controlnet_path:
        ckpt = torch.load(args.pretrained_controlnet_path,
    ...
    trainable_parameters_with_lr = {
        "params": trainable_parameters, "lr": args.learning_rate}
    ...
    def encode_video(video):
        ...
        latent_dist = vae.encode(
            video).latent_dist.sample() * vae.config.scaling_factor
    ...
    def collate_fn(examples):
        ...
        controlnet_videos = [example["controlnet_video"]
        ...
        controlnet_videos = controlnet_videos.to(
    ...
    model_config = transformer.module.config if hasattr(
        transformer, "module") else transformer.config
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                model_input = encode_video(batch["videos"]).to(
                    dtype=weight_dtype)  # [B, F, C, H, W]
                ...
                if args.enable_time_sampling:
                    if args.time_sampling_type == "truncated_normal":
                        ...
                        timesteps = torch.randint(
                            int((1 - args.controlnet_guidance_end) *
                                scheduler.config.num_train_timesteps),
                            int((1 - args.controlnet_guidance_start) *
                                scheduler.config.num_train_timesteps),
                ...
                noisy_model_input = scheduler.add_noise(
                ...
                if isinstance(controlnet_states, (tuple, list)):
                    controlnet_states = [x.to(dtype=weight_dtype)
                                         for x in controlnet_states]
                else:
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        torch.save({'state_dict': unwrap_model(
                            controlnet).state_dict()}, save_path)
            ...
            if accelerator.is_main_process:
                if args.validation_prompt is not None and (step + 1) % args.validation_steps == 0:
                    ...
                    validation_videos = args.validation_video.split(
                        args.validation_prompt_separator)
                    for validation_prompt, validation_video in zip(validation_prompts, validation_videos):
                        numpy_frames = read_video(
                            validation_video, frames_count=args.max_num_frames)
                        controlnet_frames = np.stack(
                            [train_dataset.controlnet_processor(x) for x in numpy_frames])
                        pipeline_args = {
