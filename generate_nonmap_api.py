...
from remap_pipeline import ControlnetCogVideoXPipeline, LatentAlign3D
...
class VideoGenerator:
    def __init__(self, args):
        ...
        state_dict = torch.load(
            args.lrm_weight, map_location=self.device)['model']
        state_dict = {k.replace('module.', ''): v for k,
                      v in state_dict.items()}
        lrm.load_state_dict(state_dict, strict=False)
        self.lrm = lrm
        state_dict = torch.load(args.remap_weight, map_location=self.device)
        ...
        # Load pose files
        self.pose_files = []
        for file in os.listdir(args.pose_folder):
            if file.endswith('.txt'):
                self.pose_files.append(os.path.join(args.pose_folder, file))
