...
from model.latentllrm import LongLRM
...
# save config file in checkpoint_dir
...
if rank == 0:
    ...
    for key, value in data_example.items():
        ...
        if key == 'input_images':
            ...
            input_images = input_images.permute(
                1, 2, 0, 3).flatten(2, 3)  # (C, H, V*W)
            torchvision.utils.save_image(input_images, os.path.join(
                checkpoint_dir, 'data_example', 'input_images.png'))
...
if resume_file is None:
    ...
    if isinstance(model, DDP):
        status = model.module.load_state_dict(
    ...
    if (not config.get("evaluation", False)) and (auto_resume or (not config.training.get("reset_training_state", False))):
        ...
        logger.info(
            f"Resume from train_steps_done: {train_steps_done}, param_update_steps_done: {train_steps_done // grad_accum_steps}")
        try:
            ...
            logger.warning(
                f"Failed to load optimizer and scheduler, start with new optimizer and scheduler")
...
if rank == 0 and not config.get("evaluation", False):
    ...
    os.environ["WANDB_API_KEY"] = '5a00f13b8af3c1c21596be640c46403bc57d4941'
...
if config.get("evaluation", False):
    evaluation_dir = os.path.join(
        config.evaluation_dir, config_name, '+'.join(config.data.datasets))
    if rank == 0:
    ...
    with model.no_sync(), torch.no_grad(), torch.autocast(
        ...
        for data in dataloader:
            ...
            ret_dict = model(data)
            model.module.save_evaluation_results(
                data, ret_dict, evaluation_dir)
        torch.cuda.empty_cache()
    torch.distributed.barrier()
    # aggregate results
    if rank == 0:
        ...
        logger.info(
            f"Found evaluation results for {len(scene_folders)} scenes")
        metric_dict = {}
        ...
        with open(os.path.join(evaluation_dir, "summary.csv"), "w") as f:
            ...
            for i, scene_folder in enumerate(scene_folders):
                f.write(
                    scene_folder+","+",".join([str(metric_dict[key][i]) for key in metric_dict.keys()])+"\n")
            eval_res = ""
            ...
            f.write("mean,"+",".join([str(metric_dict[key])
                    for key in metric_dict.keys()])+"\n")
            f.close()
    torch.distributed.barrier()
    exit(0)
...
while train_steps_done <= train_steps:
    ...
    if update_param and (not skip_optimizer_step):
        ...
        if config.training.grad_clip_norm > 0:
            ...
            total_grad_norm = torch.nn.utils.clip_grad_norm_(
                param_optim_list, max_norm=grad_clip_norm).item()
            if total_grad_norm > grad_clip_norm * 2.0:
                logger.warning(
                    f"step {train_steps_done} grad norm too large {total_grad_norm} > {grad_clip_norm * 2.}")
            allowed_gradnorm = grad_clip_norm * \
                config.training.get("allowed_gradnorm_factor", 5.0)
            if total_grad_norm > allowed_gradnorm:
                skip_optimizer_step = True
                logger.warning(
                    f"step {train_steps_done} grad norm too large {total_grad_norm} > {allowed_gradnorm}, skipping optimizer step")
    ...
    if rank == 0 and update_param:
        ...
        if param_update_steps_done % config.training.print_every == 0 or param_update_steps_done < param_update_steps_start + 100 or skip_optimizer_step:
            loss_str = ", ".join(
        ...
        if param_update_steps_done % config.training.checkpoint_every == 0 or train_steps_done == train_steps:
            ...
            torch.save(checkpoint, os.path.join(checkpoint_dir,
                       f"checkpoint_{param_update_steps_done:09d}.pt"))
