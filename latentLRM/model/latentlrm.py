        ...
        for i, s in enumerate(self.block_type):
            ...
            if s == "t":
                self.blocks.append(TransformerBlock(
                    dim_cur, config.model.transformer.head_dim))
                self.blocks[-1].apply(_init_weights)
            elif s == "m":
                self.blocks.append(Mamba2Block(
                    dim_cur, config.model.mamba2.d_state))
            else:
...
class LongLRM(nn.Module):
    def __init__(self, config, device):
        ...
        if config.training.get("gaussian_depth_loss", 0.0) > 0:
            ...
            self.depth_anything.load_state_dict(torch.load(
                f'model/depth_anything/depth_anything_{encoder}14.pth'))
    ...
    def render(self, xyz, feature, scale, rotation, opacity, test_c2ws, test_intr, W, H):
        ...
        for i in range(B):
            ...
            rendering, _, _ = rasterization(xyz_i, rotation_i, scale_i, opacity_i, feature_i,
                                            ...
                                            render_mode="RGB",
                                            backgrounds=torch.ones(
                                                V, 3).to(test_intr.device),
                                            rasterize_mode='classic')  # (V, H, W, 3)
            renderings.append(rendering)
        return torch.stack(renderings, dim=0)  # (B, V, H, W, 3)

    def load_video_encoder(self):
        # vae_path = os.path.join(self.config.video_model_path,'vae')
        # encoder = CogVideoXEncoder3D()
        # file_path = os.path.join(vae_path,'diffusion_pytorch_model.safetensors')  # 替换为你的 .safetensors 文件路径
        # state_dict = {}
        # with safe_open(file_path, framework="pt") as f:  # "pt" 表示 PyTorch
        #     for key in f.keys():
        #         state_dict[key] = f.get_tensor(key)
        encoder = AutoencoderKLCogVideoX.from_pretrained(
            self.config.video_model_path, subfolder='vae')
        # encoder = vae.encoder
        # 将加载的权重赋值给模型
        # encoder.load_state_dict(state_dict, strict=False)  # strict=False 允许部分加载
        # # 2. 直接提取encoder子模块
    ...
    @torch.no_grad()
    def vae_encoder(self, video):
        self.vae.eval()
        latent = self.vae.encode(video).latent_dist.sample()
        return latent

    @torch.no_grad()
    def predict(self, image_latent, H, W, input_intr, input_c2ws):
        self.eval()
        B, V, _ = input_intr.shape
        inference_start = time.time()
        device = image_latent.device
        # target_c2ws = input_c2ws.copy()
        # target_intr = input_intr.copy()
        # Embed camera info
        ...
        tokens, hh, ww = self.processor(
            tokens, self.num_global_tokens, v, hh, ww)
        # 计算完后进行下采样
        H1 = H // 2
        W1 = W // 2
        # Decode tokens
        ...
        dist, feature, scale, rotation, opacity = torch.split(
            gaussians, [1, (self.config.model.gaussians.sh_degree + 1) ** 2 * 3, 3, 4, 1], dim=-1)
        feature = feature.view(
            B, V*H1*W1, (self.config.model.gaussians.sh_degree + 1) ** 2, 3).contiguous()
        scale = (scale + self.config.model.gaussians.scale_bias).clamp(
            max=self.config.model.gaussians.scale_max)
        opacity = opacity + self.config.model.gaussians.opacity_bias

        # Align gaussian means to pixel centers
        dist = dist.sigmoid() * self.config.model.gaussians.max_dist  # (B, V*H1*W1, 1)
        ray_o, ray_d = self.get_ray(
            input_c2ws, input_intr, B, V, H1, W1, device)
        ...
        # renderings = self.render(xyz, feature, scale, rotation, opacity, input_c2ws, input_intr, W, H)

        renderings = renderings.permute(
            0, 1, 4, 2, 3).contiguous()  # (B, V, 3, H, W)
    ...
    def forward(self, input_dict):
        ...
        # Pachify
        # VAE编码
        image_latent = self.vae_encoder(
            rearrange(input_images, "b v c h w -> b c v h w"))
        camera_embedding = camera_embedding.to(image_latent.dtype)
        # Token化处理
        image_tokens, v, ww, hh = self.tokenizer(
            image_latent, camera_embedding)  # 将图像和相机信息转换为tokens
        ...
        if self.config.training.get("gaussian_depth_loss", 0.0) > 0:
            pos = xyz.reshape(B, V, H1*W1, 3)
            input_w2cs = input_c2ws.float().inverse()  # (B, V, 4, 4)
            pos_cam = pos @ input_w2cs[:, :, :3, :3].transpose(-1, -2).contiguous(
            ) + input_w2cs[:, :, :3, 3:4].transpose(-1, -2).contiguous()  # (B, V, H*W, 3)
            depth_pred = pos_cam[..., 2]  # (B, V, H1*W1)
            disp_pred = 1.0 / depth_pred.clamp(min=1e-2)
            disp_median = torch.median(
                disp_pred, dim=-1, keepdim=True)[0]  # (B, V, 1)
            disp_var = (disp_pred - disp_median).abs().mean(dim=-
                                                            1, keepdim=True)  # (B, V, 1)
        ...
        gaussian_usage = (opacity.sigmoid() > self.config.model.gaussians.get(
            "opacity_threshold", 0.001)).float().mean(dim=1).squeeze(-1)  # (B,)
        if prune_ratio > 0:
            ...
            if num_keep_random > 0:
                ...
                random_idx = rest_idx[:, torch.randperm(
                    rest_idx.shape[1])[:num_keep_random]]
        ...
        with torch.autocast(enabled=False, device_type="cuda"):
            if use_checkpoint:
                ...
                renderings = self.render(
                    xyz, feature, scale, rotation, opacity, test_c2ws, test_intr, W, H)  # (B, V, H, W, 3)
        renderings = renderings.permute(
            0, 1, 4, 2, 3).contiguous()  # (B, V, 3, H, W)
        ret_dict["renderings"] = renderings
        ...
        test_images = test_images.flatten(0, 1)
        # test_images = nn.functional.interpolate(test_images.flatten(0, 1), (H, W), mode='bilinear') # (B*V, 3, H_, W_)
        ...
        if self.config.model.gaussians.get("align_to_pixel", True) and self.config.training.get("gaussian_depth_loss", 0.0) > 0:
            H_ = (H1 // 14) * 14
            W_ = (W1 // 14) * 14
            input_images_ = nn.functional.interpolate(input_images.flatten(
                0, 1), (H_, W_), mode='bilinear')  # (B*V, 3, H_, W_)
            with torch.no_grad():
                self.depth_anything.eval()
                disp_da = self.depth_anything(input_images_).reshape(
                    B, V, H_, W_)  # (B, V, H_, W_)
            disp_da = nn.functional.interpolate(disp_da, (H1, W1), mode='bilinear').to(
                disp_pred.dtype).reshape(B, V, H1*W1)  # (B, V, H*W)
            disp_median = torch.median(
                disp_da, dim=-1, keepdim=True)[0]  # (B, V, 1)
            disp_var = (disp_da - disp_median).abs().mean(dim=-
                                                          1, keepdim=True)  # (B, V, 1)
            ...
            ret_dict["disp_da"] = disp_da.reshape(B, V, H1, W1)
            ret_dict["disp_pred"] = disp_pred.reshape(B, V, H1, W1)
    ...
    def save_gaussian_ply(self, gaussian_dict, save_path, opacity_threshold=None):
        ...
        N = xyz.shape[0]
        feature = gaussian_dict["feature"].detach(
        ).cpu().float()  # (N, (sh_degree+1)**2, 3)
        f_dc = feature[:, 0].contiguous()  # (N, 3)
        f_rest_full = torch.zeros(N, 3*(3+1)**2-3).float()
        if feature.shape[1] > 1:
            f_rest = feature[:, 1:].transpose(1, 2).reshape(
                N, -1)  # (N, 3*(sh_degree+1)**2-3)
        ...
        if opacity_threshold is not None:
            attributes = attributes[opacity.squeeze(
        ...
        attribute_list += ['rot_{}'.format(i)
                           for i in range(rotation.shape[1])]
        dtype_full = [(attribute, 'f4') for attribute in attribute_list]
        dtype_full[3:6] = [(attribute, 'u1')
                           for attribute in attribute_list[3:6]]
    ...
    def save_input_video(self, input_intr, input_c2ws, gaussian_dict, H, W, save_path, insert_frame_num=8):
        ...
        input_c2ws = torch.cat(
            [input_c2ws, input_c2ws[:1]], dim=0)  # wrap around
        input_intr_mat = torch.cat(
            [input_intr_mat, input_intr_mat[:1]], dim=0)  # wrap around
        c2ws, intr_mat, _ = get_interpolated_poses_many(
            input_c2ws[:, :3, :4], input_intr_mat, steps_per_transition=insert_frame_num)
        V = c2ws.shape[0]
        ...
        feature = gaussian_dict["feature"].detach().float().to(
            device)  # (N, (sh_degree+1)**2, 3)
        scale = gaussian_dict["scale"].detach().float().to(device)  # (N, 3)
        rotation = gaussian_dict["rotation"].detach(
        ).float().to(device)  # (N, 4)
        opacity = gaussian_dict["opacity"].detach(
        ).float().to(device).squeeze(-1)  # (N, 1)
        ...
        with torch.autocast(enabled=False, device_type="cuda"):
            for i in range(V):
                rendering = GaussianRenderer.render(xyz, feature, scale, rotation, opacity,
                                                    ...
                                                    self.config.model.gaussians.far_plane)
                rendering = rendering.squeeze(0).clamp(
                    0, 1).cpu().numpy()  # (H, W, 3)
        ...
        video_writer = cv2.VideoWriter(
            tmp_save_path, cv2.VideoWriter_fourcc(*'mp4v'), 30, (W, H))
        for r in renderings:
            video_writer.write(r)
        video_writer.release()
        subprocess.run(
            f"ffmpeg -y -i {tmp_save_path} -vcodec libx264 -f mp4 {save_path}", shell=True)
    ...
    def save_visualization(self, input_dict, output_dict, save_dir, save_gaussian=False, save_video=False):
        ...
        input_image = input_images[0].permute(
            1, 2, 0, 3).flatten(2, 3)  # (3, H, Vin*W)
        ...
        target_renderings = []
        for i in range(B):
            target_image = target_images[i].permute(
                1, 2, 0, 3).flatten(2, 3)  # (3, H, V*W)
            rendering_image = renderings[i].permute(
                1, 2, 0, 3).flatten(2, 3)  # (3, H, V*W)
            target_renderings.append(target_image)
            target_renderings.append(rendering_image)
        target_rendering = torch.cat(
            target_renderings, dim=1)  # (3, 2*B*H, V*W)
        torchvision.utils.save_image(target_rendering, target_rendering_path)

        if "disp_da" in output_dict:
            cmapper = plt.colormaps['magma']
            disp_da = output_dict["disp_da"][0].detach().permute(
                1, 0, 2).flatten(1, 2)  # (V, H, W) -> (H, V*W)
            disp_pred = output_dict["disp_pred"][0].detach().permute(
                1, 0, 2).flatten(1, 2)  # (V, H, W) -> (H, V*W)
            ...
            disp_image = torch.cat([torch.tensor(disp_da), torch.tensor(
                disp_pred)], dim=0).permute(2, 0, 1)  # (3, 2*H, V*W)
        ...
        if save_gaussian:
            ...
            opacity_threshold = self.config.model.gaussians.get(
                "opacity_threshold", 0.001)
            self.save_gaussian_ply(gaussian_first, os.path.join(
                save_dir, f"gaussians_{str(opacity_threshold).split('.')[-1]}.ply"), opacity_threshold)
    ...
    def save_evaluation_results(self, input_dict, output_dict, save_dir):
        ...
        for i in range(B):
            ...
            with open(os.path.join(scene_dir, "metrics.csv"), "w") as f:
                ...
                for j in range(V):
                    f.write(
                        f"{j}, {psnr[j].item()}, {ssim[j].item()}, {lpips[j].item()}\n")
                f.write(
                    f"mean, {psnr.mean().item()}, {ssim.mean().item()}, {lpips.mean().item()}\n")
            ...
            input_image = input_images[i].permute(
                1, 2, 0, 3).flatten(2, 3)  # (3, H, Vin*W)
            ...
            for j in range(V):
                target_path = os.path.join(scene_dir, "images", f"{j}.png")
                rendering_path = os.path.join(
                    scene_dir, "rendering", f"{j}.png")
            ...
            opacity_threshold = self.config.model.gaussians.get(
                "opacity_threshold", 0.001)
            self.save_gaussian_ply(gaussian, os.path.join(
                scene_dir, f"gaussians_{str(opacity_threshold).split('.')[-1]}.ply"), opacity_threshold)
            ...
            self.save_input_video(input_intr, input_c2ws, gaussian, H, W, os.path.join(scene_dir, "input_traj.mp4"),
                                  insert_frame_num=self.config.get("insert_frame_num", 8))
            # save rotation matrix and T and FoVx,FoVy as json
            self.save_poses_as_json(
                input_c2ws, input_intr, H, W, os.path.join(scene_dir, "cameras"))

    def save_poses_as_json(self, input_c2ws, input_intr, H, W, save_folder):
        os.makedirs(save_folder, exist_ok=True)
        Rs = input_c2ws.transpose(-1, -2)[:, :3, :3]
        Ts = input_c2ws[:, :3, 3]
        FoVxs = 2 * torch.atan(W / (2 * input_intr[:, 0]))
        FoVys = 2 * torch.atan(H / (2 * input_intr[:, 1]))
        for i, (R, T, FoVx, FoVy) in enumerate(zip(Rs, Ts, FoVxs, FoVys)):
            with open(os.path.join(save_folder, f"{i}.json"), 'w') as f:
                json.dump({"R": R.tolist(), "T": T.tolist(),
                          "FoVx": FoVx.item(), "FoVy": FoVy.item()}, f)
...
if __name__ == "__main__":
    ...
    input_intr[:, :, 2] = w//2
    input_intr[:, :, 3] = h//2
    input_c2ws = torch.eye(4).unsqueeze(0).expand(
        batch_size, view, 4, 4).to("cuda")
    ...
    test_intr[:, :, 2] = w//2
    test_intr[:, :, 3] = h//2
    test_c2ws = torch.eye(4).unsqueeze(0).expand(
        batch_size, view_test, 4, 4).to("cuda")
    ...
    loss_total.backward()
    print("Peak memory with checkpoint:",
    ...
    print("Peak memory without checkpoint:",
          torch.cuda.max_memory_allocated() / 1024 / 1024, "MB")
