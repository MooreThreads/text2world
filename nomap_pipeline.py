...
from reward.latent_lrm import LongLRM
...
class ControlnetCogVideoXPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def __init__(
        ...
        self.register_modules(
            tokenizer=tokenizer, text_encoder=text_encoder, vae=vae, transformer=transformer, controlnet=controlnet, scheduler=scheduler, lrm=lrm
        )
    ...
    def __call__(
        ...
        # postprocess decoder video
        pvideo = self.video_processor.postprocess_video(
            video=video, output_type='pt')
        # Offload all models
        self.maybe_free_model_hooks()
        video = nvideo.permute(0, 2, 1, 3, 4)  # [B, C, F, H, W]
        vae_latents = self.vae.encode(
            video).latent_dist.sample() * self.vae.config.scaling_factor

        # generate render video
        renderings = self.lrm((1, num_frames, 3, height, width),
                              image_latent=vae_latents, input_c2ws=c2ws, input_intr=intrinsics)
        renderings = renderings.clamp(0, 1)  # (B,  3, V, H, W)
        return pvideo, renderings
